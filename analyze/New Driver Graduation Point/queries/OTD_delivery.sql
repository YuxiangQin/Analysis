...
dlvr as (
    select *
    from (select *, row_number() over (partition by drvr_user_id order by CARRIER_DLVR_TS_TZ) as dlvr_rk
        from base)
    where dlvr_rk <= TOP_N
)
...
select dlvr_rk,
    ...
    count(distinct SALES_ORDER_NUM || po_equ_nbr || trip_id) as tot_dlvr_cnt,
    count(distinct case when On_Time_Delivery = 'Y' then SALES_ORDER_NUM || po_equ_nbr || trip_id end
        ) as on_tm_dlvr_cnt, 
    count(distinct case when On_Time_Delivery = 'Y' then SALES_ORDER_NUM || po_equ_nbr || trip_id end)
      / count(distinct SALES_ORDER_NUM || po_equ_nbr || trip_id) as OTD
from dlvr 
group by 1
order by 1